...
def test_malformed_toml_raises_value_error_with_path(tmp_path: Path) -> None:
    ...
    assert "profile.toml is not valid TOML" in str(excinfo.value)
    assert str(tmp_path / "profile.toml") in str(excinfo.value)
...
def test_missing_profile_file_fails_closed(tmp_path: Path) -> None:
    ...
    assert "profile.toml" in str(excinfo.value)
